    ...
    const fetchClaimStatus = async () => {
        try {
            ...
            if (!response.payload.isAvailable) {
                ...
                const minutesRegex = /(\d+) minutes/;
                const match = nextClaimTime.match(minutesRegex);
                if (match && match[1]) {
                ...
                <CardContent mt={15}>
                    ...
                    {countdown ? (
                        <Stack direction="column" spacing={2} >
                            <Typography variant='h5'>Next claim in {countdown}</Typography>
                        </Stack>
                    ) : (
                        <LoadingButton
                            variant='contained'
                            ...
                            sx={{
                                p: theme => theme.spacing(1.75, 5.5), opacity: claimStatus?.isAvailable ? 1 : 0.5,
                                pointerEvents: claimStatus?.isAvailable ? 'auto' : 'none',
                            }}
